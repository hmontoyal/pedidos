var BASE_URL = window.location.origin+'/pedidos';
...
$('.clone-wrapper').cloneya().on('clone_after_append', function(event, toclone, newclone) {
    $(newclone).find('.bootstrap-select').replaceWith(function() { return $('select', this); });
    $(newclone).find('.selectpicker').selectpicker();
    //$(newClone).find('.selectpicker').selectpicker('destroy');
  // $(newclone).find('.selectpicker').selectpicker('refresh');
 // alert('ok');
});
